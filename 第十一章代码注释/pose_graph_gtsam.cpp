...
int main ( int argc, char** argv )
{
    ...
    while ( !fin.eof() )
    {
        ...
        else if ( tag == "EDGE_SE3:QUAT" )
        {
            ...
            // g2o的信息矩阵定义方式与gtsam不同，这里对它进行修改（讲对角线的块进行互换）
    ...
    params_lm.setVerbosity("ERROR");  // 输出优化信息
    params_lm.setMaxIterations(20); // 最大迭代次数
    params_lm.setLinearSolverType("MULTIFRONTAL_QR"); // 线性求解器类型
    // 构建优化器，传入的参数为先验图，初始值，优化器参数
