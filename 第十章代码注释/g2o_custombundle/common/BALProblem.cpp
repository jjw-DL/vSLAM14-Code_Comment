...
BALProblem::BALProblem(const std::string &filename, bool use_quaternions)
{
  ...
  if (use_quaternions)
  {
    ...
    // 变换角轴到四元数后，相机参数由9变为了10，num_parameters_为重新计算的总参数量
    num_parameters_ = 10 * num_cameras_ + 3 * num_points_;
    // 构造一个大小为总参数量的新数组，记为四元数参数
...
//构造函数读入数据txt，将数据存入类成员中。这里是反向过程，由类成员中存储的数据，生成一个优化后的.txt数据。
                                            ...
                                            double *center) const
{
  ...
  //这里用指针偏移取得t的值，先偏移到末尾，再往后倒数第6个，为平移数据的开始
  AngleAxisRotatePoint(inverse_rotation.data(),
                       camera + camera_block_size() - 6,
                       center);

  //最后加上负号。记住，map类型构造的是引用，能直接对原构造数组进行操作。
...
void BALProblem::Normalize()
{
  ...
  // 这里不是计算路标的质心，质心的话是各维的平均值。这里用的是Median()函数，此函数返回的是中位值！
                         ...
                         const double point_sigma)
{
  ...
  for (int i = 0; i < num_cameras_; ++i)
  {
    ...
    // representation
    // 从camera中取出三维的angle_axis
    CameraToAngelAxisAndCenter(camera, angle_axis, center);
    if (rotation_sigma > 0.0)
    {
      PerturbPoint3(rotation_sigma, angle_axis);
    }
    //添加噪声完后再用AngleAxisAndCenterToCamera()重构camera参数
    AngleAxisAndCenterToCamera(angle_axis, center, camera);
